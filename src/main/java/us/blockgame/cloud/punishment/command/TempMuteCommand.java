...
public class TempMuteCommand {
    ...
    @SneakyThrows
    @Command(name = "tempmute", permission = "cloud.command.mute", runAsync = true)
    public void tempMute(CommandSender sender, @Param(name = "player") OfflinePlayer offlinePlayer, @Flag(value = {"s", "silent"}) boolean silent, @Param(name = "duration") String duration, @Param(name = "reason", wildcard = true, optional = true, defaultValue = "Misconduct") String reason) {
        ...
        //Broadcast ban
        Bukkit.getOnlinePlayers().stream().filter(p -> (silent ? p.hasPermission("cloud.staff") : true)).forEach(p -> p.sendMessage((silent ? ChatColor.GRAY + "(Silent) " : "") + ChatColor.GREEN + offlinePlayer.getName() + " has been" + (time != -1L ? " temporarily" : "") + " muted by " + sender.getName() + " for " + reason + "."));
        return;
    }
}
